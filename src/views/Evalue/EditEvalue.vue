...
const rating = ref(0)
const content = ref('')
const evalueId = Number(route.params.id)
...
const fetchEvalueDetails = async () => {
  try {
    if (!evalueId) {
      throw new Error('ID đánh giá không hợp lệ')
    }
    const evalue = await evalueService.getEvalueById(evalueId)
    rating.value = evalue.rating
    content.value = evalue.content || ''
    
    // Tìm và set advice tương ứng
    const advice = advices.value.find(a => a.advice_id === evalue.advice_id)
    if (advice) {
...
const handleSubmit = async () => {
  if (!currentUser.value) {
    ElMessage.warning('Vui lòng đăng nhập để chỉnh sửa đánh giá')
    router.push('/login')
    return
  }
  ...
  try {
    ...
    await evalueService.updateEvalue(evalueId, {
...
onMounted(async () => {
  await fetchUserAdvices()
  await fetchEvalueDetails()
})
</script>

<template>
  <div class="edit-evalue-container">
    <div class="header">
